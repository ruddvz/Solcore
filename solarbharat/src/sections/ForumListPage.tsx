  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white">{t('forum.title')}</h1>
          <p className="mt-2 max-w-2xl text-sm text-white/55">{t('forum.subtitle')}</p>
        </div>
        <Link
          href="/forum/new"
      ...
      {loading ? (
        <p className="text-sm text-white/45">{t('forum.loading')}</p>
      ) : (
        <div className="space-y-3">
          {topics.map((topic) => (
            <Link key={topic.id} href={`/forum/topic?slug=${encodeURIComponent(topic.slug)}`}>
      ...
      {!loading && topics.length === 0 && (
        <p className="text-sm text-white/45">{t('forum.empty')}</p>
      )}

      <p className="text-xs text-white/40">{t('forum.disclaimer')}</p>
    </div>
  )
}
