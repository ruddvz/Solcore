  ...
  if (!ok) {
    return (
      <div className="space-y-4">
        <p className="text-sb-muted">{t('phase3.notFound')}</p>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <div>
        ...
        <h1 className="mt-4 text-3xl font-black text-sb-ink">{t(`phase3.detail.${slug}.title`)}</h1>
        <p className="mt-3 text-sm leading-relaxed text-sb-muted">{t(`phase3.detail.${slug}.body`)}</p>
      </div>

      <div className="rounded-2xl border border-sb-line bg-sb-surface p-6">
      ...
      <p className="text-xs text-sb-muted">{t(`phase3.detail.${slug}.note`)}</p>
    </div>
  )
}
